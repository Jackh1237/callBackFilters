...
console.log("Search for Alexis's phone number ")
...
function searchUser(phonebook,searchName, getNumber){
    for ( let x of phonebook)
        if (x.name == searchName)
            console.log(searchName +"'s " + getNumber(x))
}


// TODO: Create a callback function called getNumber() to find the recipient's number.

function getNumber(person){
    return "phone number: " + person.phone 
}


// TODO: Call the searchUser() function and pass getNumber() as a callback function.
searchUser(phoneBook, "Alexis",getNumber)
